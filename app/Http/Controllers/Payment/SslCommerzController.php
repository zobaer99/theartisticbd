...
class SslCommerzController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Fixed currency for BDT
        $currency = (object) ['name' => 'BDT', 'sign' => '৳', 'value' => 1];
        ...
        if (!PriceHelper::Digital()) {
            ...
            $billingInfo = [
            'bill_first_name' => $request->input('ship_full_name', $request->input('ship_full_name')),
            'bill_last_name'  => $request->input('ship_full_name', $request->input('ship_full_name')),
            'bill_email'      => $request->input('bill_email', $request->input('ship_email')),
            'bill_phone'      => $request->input('bill_phone', $request->input('ship_phone')),
            'bill_company'    => $request->input('bill_company'),
            'bill_address1'   => $request->input('bill_address1', $request->input('ship_address')),
            'bill_address2'   => $request->input('bill_address2', $request->input('ship_address2')),
            'bill_zip'        => $request->input('bill_zip', $request->input('ship_zip')),
            'bill_city'       => $request->input('bill_city', $request->input('ship_city')),
            'bill_country'    => $request->input('bill_country', $request->input('ship_country')),
        ];

        $shippingInfo = [
            'ship_first_name' => $request->input('ship_full_name', $billingInfo['bill_first_name']),
            'ship_last_name'  => $request->input('ship_full_name', $billingInfo['bill_last_name']),
            'ship_email'      => $request->input('ship_email', $billingInfo['bill_email']),
            'ship_phone'      => $request->input('ship_phone', $billingInfo['bill_phone']),
            'ship_company'    => $request->input('ship_company'),
            'ship_address1'   => $request->input('ship_address'),
        ...
        $bill_info = Session::get('shipping_address');
        # CUSTOMER INFORMATION
        $post_data['cus_name'] = $billingInfo['bill_first_name'];
        $post_data['cus_email'] = $billingInfo['bill_email'];
        $post_data['cus_add1'] = '';
        $post_data['cus_city'] = '';
        $post_data['cus_postcode'] = '';
        $post_data['cus_country'] = '';
        $post_data['cus_phone'] = $billingInfo['bill_phone'];
        $post_data['cus_fax'] = '';
        ...
        if ($input['status'] == 'VALID') {
            ...
            if (isset($order)) {
                ...
                $emailData = [
                    ...
                    'type' => "Order",
                    'user_name' => isset($user) ? $user->displayName() : Session::get('shipping_address')['ship_first_name'],
