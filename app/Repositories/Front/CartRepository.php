...
class CartRepository
{
    ...
    $single = isset($request->type) ? ($request->type == '1' ? 1 : 0) : 0;
        ...
        if (isset($cart[$item->id . '-' . $cart_item_key])) {
            ...
            // Determine if this is an absolute quantity update (from + / - controls)
            // Front-end passes existing cart item_key when changing quantity; treat as set, not increment.
            if ($qty_check == 1 || (isset($request->absolute_qty) && $request->absolute_qty != (int)0) || (isset($input['absolute_qty']) && $input['absolute_qty'] != (int)0)) {
                $cart[$item->id . '-' . $cart_item_key]['qty'] =  max(1, (int)$qty);
            } else {
