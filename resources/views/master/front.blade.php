...
<head>
    ...
    @if (url()->current() == route('front.index'))
        ...
        <meta property="og:image:secure_url" content="{{ url('/storage/images/' . $setting->meta_image) }}" />
        ...
        <meta name="facebook-domain-verification" content="ov9d1yz2bequop5gl9i8z41znrrm6g" />
    @else
        @yield('meta')
    @endif
    ...
    <style>
        ...
        .whatsapp-chat {
            position: fixed;
            bottom: 125px; /* Increased spacing to avoid overlap */
            right: 8px;
            z-index: 999;
        }
        ...
        @media (max-width: 768px) {
            .whatsapp-chat {
                bottom: 125px; /* Adjusted for mobile spacing */
                right: 15px;
            }
...
">
                    ...
                    <div class="col-md-8">
                        <div class="right-area">
                            ...
                            <div class="login-register ">
                                @if (!Auth::user())
                                    <a class="track-order-link login-button mr-0" href="{{ route('user.login') }}">

                                        <div class="main-link">
                                            <i class="icon-user pr-2"></i> <span class="text-label">Login</span>
                                        </div>
                                    ...
                                    <div class="t-h-dropdown">
                                        ...
                                        <div class="t-h-dropdown-menu">
                                            <a href="{{ route('user.dashboard') }}"><i
                                                    class="fas fa-home"></i>{{ __('Dashboard') }}</a>
                                            <a href="{{ route('user.logout') }}"><i
                                                    class="fas fa-power-off pr-2"></i>{{ __('Logout') }}</a>
                                        </div>
                                    </div>
                                @endif
                            </div>
                                            ...
                                            <ul>
                                                ...
                                                @if ($setting->is_shop == 1)
                                                    <li
                                                        ...
                                                        <a href="{{ route('front.catalog') }}"><i
                                                                class="icon-chevron-right"></i>Explore Products</a>
                                                    </li>
                                                @endif
                                                @if ($setting->is_campaign == 1)
                                                    <li
                                                        ...
                                                        <a href="{{ route('front.campaign') }}"><i
                                                                class="icon-chevron-right"></i>Special Offers</a>
                                                    </li>
                                                @endif
                                                ...
                                                {{-- <li class="t-h-dropdown">
                                                    ...
                                                    <div class="t-h-dropdown-menu">
                                                        ...
                                                        @endif
                                                        @foreach ($footer_pages->where('pos', 0)->merge($footer_pages->where('pos', 2)) as $page)
            ...
            <div class="row">
                    ...
                    <section class="widget widget-light-skin">
                        <div class="site-branding mb-4">
                            <a class="site-logo align-self-center" href="{{ route('front.index') }}">
                                <img src="{{ url('/storage/images/' . $setting->logo) }}"
                                    alt="{{ $setting->title }}">
                            </a>
                        </div>
                        <p class="mb-1"><i class="icon-map-pin me-2"></i><strong>{{ __('Address') }}: </strong>
                            {{ $setting->footer_address }}</p>
                        <p class="mb-1"><i class="icon-phone me-2"></i><strong>{{ __('Phone') }}: </strong>
                            {{ $setting->footer_phone }}</p>
                        <p class="mb-1"><i class="icon-mail me-2"></i><strong>{{ __('Email') }}: </strong>
                ...
                <div class="col-lg-3 col-sm-6">
                    ...
                    <div class="widget widget-links widget-light-skin">
                        ...
                        @php $categories = collect($allCategories ?? [])->take(10); @endphp
                        <ul>
                            @foreach ($categories as $category)
                                <li><a
                                        href="{{ route('front.catalog') . '?category=' . $category->slug }}">{{ $category->name }}</a>
                                </li>
                            @endforeach
                ...
                <div class="col-lg-3 col-sm-6">
                    ...
                    <div class="widget widget-links widget-light-skin">
                        ...
                        <ul>
                            ...
                            @endif
                            @foreach ($footer_pages->whereIn('pos', [1, 2]) as $page)
    ...
    @php
        ...
        $is_whatsapp_enabled = $setting->is_whatsapp_enabled ?? true;
        $whatsapp_message = $setting->whatsapp_message ?: 'Hello, I need help with your products';
        $encoded_message = urlencode($whatsapp_message);
    @endphp
    @if($whatsapp_clean && $is_whatsapp_enabled)
    <div id="whatsapp-chat" class="whatsapp-chat">
        <a href="https://wa.me/{{ $whatsapp_clean }}?text={{ $encoded_message }}" 
           target="_blank" 
