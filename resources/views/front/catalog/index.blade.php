            ...
            <aside class="sidebar sidebar-offcanvas position-left"><span class="sidebar-close"><i class="icon-x"></i></span>
              ...
              <section class="widget widget-categories card rounded p-4">
                ...
                <ul id="category_list" class="category-scroll">
                 @foreach ($categories_all as $cat)
                  <li class="has-children {{ isset($category) && $category->id == $cat->id ? 'expanded active' : '' }}">
                      <a class="category_search" href="javascript:;" data-href="{{ $cat->slug }}">
                          {{ $cat->name }} ({{ $cat->items_count??'' }})
                      </a>

                      <ul id="subcategory_list">
                          @if($cat->subcategory->count()??0)
                              @foreach ($cat->subcategory as $getsubcategory)
                                  <li class="{{ isset($subcategory) && $subcategory->id == $getsubcategory->id ? 'active' : '' }}">
                                      ...
                                      @if($getsubcategory->childcategory->count())
                                          <ul id="childcategory_list">
                                              @foreach ($getsubcategory->childcategory as $getchildcategory)
                                                  <li class="{{ isset($childcategory) && $childcategory->id == $getchildcategory->id ? 'active' : '' }}">
              ...
              <section class="widget widget-categories card rounded p-4">
                ...
                <div class="custom-control custom-checkbox">
                  <input class="custom-control-input option" {{isset($subcategoryt) && $subcategoryt->id == $option->id ? 'checked' : ''}}   type="checkbox" value="{{$option->name}}" id="{{$attrubute->id}}{{$option->name}}">
