    ...
    $(document).ready(function () {
        ...
        $("[data-date-time]").each(function () {
            ...
            $this.countdown(finalDate, function (event) {
                $this.html(
                    event.strftime(
                        `<span class="countdown-days">%D<small>${language.Days}</small></span></small> <span class="countdown-days">%H<small>${language.Hrs}</small></span> <span class="countdown-days">%M<small>${language.Min}</small></span> <span class="countdown-days">%S<small>${language.Sec}</small></span>`
                    )
                );
            });
        });
        ...
        $(document).on("click", ".increaseQty", function () {
          ...
          getData();
                        ...
                        $(document).on("click", ".increaseQtycart", function (e) {
            e.preventDefault(); // Prevent any default action
            let item_key = $(this).attr('data-target');
            let item_id = $(this).attr('data-id');
            let item = $(this).attr('data-item');
            let newOptionArray = item.split(',');
            let currentQty = parseInt($(this).parent().find('input').val());
            let newQty = currentQty + 1;  // Simple increment by 1
            $(this).parent().find('input').val(newQty); // Update input immediately
            cartSubmit(item_key, item_id, newQty, newOptionArray, true);
        ...
        $(document).on("click", ".decreaseQty", function () {
            getData();
                ...
                $(document).on("click", ".decreaseQtycart", function (e) {
            e.preventDefault(); // Prevent any default action
            let item_key = $(this).attr('data-target');
            let item_id = $(this).attr('data-id');
            let currentQty = parseInt($(this).parent().find('input').val());
            let newQty = currentQty - 1; // Simple decrement by 1

            if (newQty > 0) {
                $(this).parent().find('input').val(newQty); // Update input immediately
                cartSubmit(item_key, item_id, newQty, null, true);
            }
            return false; // Prevent event bubbling
        });
        ...
        function cartSubmit(item_key, item_id, cartQty, newOptionArray, absolute = false) {
            // Prevent page reload/redirect for cart updates
            if (event) {
        ...
        function validateAttributes() {
            ...
            let colorAttribute = $('input[data-validation="color"]:checked');
            if ($('input[data-validation="color"]').length && !colorAttribute.length) {
                isValid = false;
                missingAttributes.push('Color');
            }
            
            // Check size attributes
            let sizeAttribute = $('input[data-validation="size"]:checked');
            if ($('input[data-validation="size"]').length && !sizeAttribute.length) {
                isValid = false;
                missingAttributes.push('Size');
            }
            
            if (!isValid) {
                dangerNotification('Please select: ' + missingAttributes.join(' and '));
            }
        ...
        function getData(status = 0, check = 0, item_key = 0, qty = 0, add_type = 0, optionIds = null) {
            // For add to cart and buy now, validate attributes
            if (status === 1 && !validateAttributes()) {
                return false;
            }
            ...
            // Collect attribute ids from checked radio inputs
            let attribute_ids = $(".attribute_option:checked")
                .map(function () {
            ...
            if (optionIds != null) {
                ...
                var options_ids = $(".attribute_option:checked")
                    .map(function () {
            ...
            let setCurrency = "৳"; // Fixed currency sign
           ...
           let shippingInput = $('input[name="shipping_id"]:checked'); 
            let shippingPrice = parseFloat(shippingInput.data('price')) || 0;

            if (shippingPrice) {
                mainPrice = mainPrice + shippingPrice;
            }
                mainPrice = number_format(mainPrice, 2, decimal_separator, thousand_separator);
            if (currency_direction == 0) {
                $('#main_price').html(mainPrice);
            } else {
                $('#main_price').html(mainPrice);
            }

            if (status == 1) {
                let addToCartUrl = `${mainurl}/product/add/cart?item_id=${itemId}&options_ids=${options_ids}&attribute_ids=${attribute_ids}&quantity=${quantity}&type=${type}&item_key=${item_key}&add_type=${add_type}&absolute_qty=1`;
                $.ajax({
                    method: "GET",
                    contentType: false,
                    cache: false,
                    ...
                    success: function (data) {
                        ...
                        if (data.status == 'outStock') {
                            ...
                            // Update cart count and header
                            $(".cart_count").text(data.qty);
                            $(".cart_view_header").load($("#header_cart_load").attr("data-target"));
                            
                            // For quantity updates (when we have an item_key)
                            if (item_key) {
                                $("#view_cart_load").load($("#cart_view_load").attr("data-target"), function() {
                                    successNotification(data.message || 'Cart updated successfully');
                                });
                            }
                            // For "Buy Now" action only (no item_key and add_type is 1)
                            else if (add_type == 1 && !item_key) {
                                location.href = mainurl + '/checkout/billing/address';
                            }
                            // For other cart actions
                            else {
                                successNotification(data.message);
                            }
                        }
        ...
        function optionPrice() {
            // Retrieve price adjustments from checked attribute radio inputs
            let option_prices = $(".attribute_option:checked")
                .map(function () {
