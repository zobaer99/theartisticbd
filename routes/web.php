...
use App\Http\Controllers\Back\VedioStorieController;
...
Route::group(['middleware' => ['adminlocalize', 'demo']], function () {
    Route::prefix('admin')->group(function () { 
        ...
        Route::get('/dashboard/data', 'Back\AccountController@getDashboardData')->name('back.dashboard.data');
        Route::post('/dashboard/data', 'Back\AccountController@getDashboardData')->name('back.dashboard.data.post');
        Route::get('/calendar/events', 'Back\AccountController@getCalendarEvents')->name('back.calendar.events');
        ...
        Route::post('/clear-cache/{type?}', function ($type = 'all') {
            \App\Helpers\CacheHelper::clearAllGlobalCache();
            ...
            switch($type) {
                case 'settings':
                    ...
                    break;
                case 'categories':
                    \App\Helpers\CacheHelper::clearCategoriesCache();
                    break;
                case 'brands':
                    \App\Helpers\CacheHelper::clearBrandsCache();
                    break;
                case 'products':
                    \App\Helpers\CacheHelper::clearProductsCache();
                    break;
                case 'sliders':
                    \App\Helpers\CacheHelper::clearSlidersCache();
                    break;
                default:
                    \App\Helpers\CacheHelper::clearAllGlobalCache();
                    break;
        ...
        Route::get('bulk/pathao/send', 'Back\BulkDeleteController@bulkPathaoSend')->name('back.bulk.pathao');
        Route::post('bulk/pathao/status', 'Back\BulkDeleteController@getPathaoStatus')->name('back.bulk.pathao.status');
        ...
        Route::group(['middleware' => 'permissions:Manage Orders'], function () {
            ...
            Route::get('/order/create', 'Back\OrderController@create')->name('back.order.create');
            Route::post('/order/store', 'Back\OrderController@store')->name('back.order.store');
            Route::get('/order/show/{id}', 'Back\OrderController@show')->name('back.order.show');
            ...
            Route::get('/order/product-details/{id}', 'Back\OrderController@getProductDetails')->name('back.order.product.details');
        });
        ...
        Route::group(['middleware' => 'permissions:Ecommerce'], function () {
            ...
            // Address-based shipping CRUD
            Route::resource('shipping-location', 'Back\ShippingLocationPriceController', ['as' => 'back', 'except' => 'show']);
            Route::get('shipping-location/districts', 'Back\ShippingLocationPriceController@districts')->name('back.shipping-location.districts');
            Route::get('shipping-location/thanas', 'Back\ShippingLocationPriceController@thanas')->name('back.shipping-location.thanas');

            //------------ CURRENCY (Commented out - using fixed BDT currency) ------------
        ...
        Route::group(['middleware' => 'permissions:Manages Pages'], function () {
            ...
            Route::resource('video-stories', VedioStorieController::class,['as' => 'back', 'except' => 'show']);
        ...
        Route::group(['middleware' => 'permissions:Manage Site'], function () {
            ...
            // Admin test endpoint for Facebook CAPI
            Route::post('/setting/facebook-capi/test', 'Back\SettingController@sendTestCapiEvent')->name('back.setting.facebook_capi.test');
...
Route::group(['middleware' => 'maintainance'], function () {
    Route::group(['middleware' => 'localize'], function () {
        ...
        // Currency system commented out - using fixed BDT currency
        ...
        // Cart attribute update routes
        Route::post('/cart/update-attribute', 'Front\CartController@updateAttribute')->name('cart.update.attribute');
        Route::get('/cart/get-attribute-options', 'Front\CartController@getAttributeOptions')->name('cart.get.attribute.options');
    ...
    // Address-based shipping helpers
    Route::get('/checkout/districts', 'Front\CheckoutController@getDistricts')->name('front.checkout.districts');
    Route::get('/checkout/thanas', 'Front\CheckoutController@getThanas')->name('front.checkout.thanas');
    Route::get('/checkout/address-shipping/setup', 'Front\CheckoutController@addressShippingSetUp')->name('front.address.shipping.setup');
    
    // Pathao API routes
    Route::get('/pathao/cities', 'Front\CheckoutController@getPathaoCities')->name('front.pathao.cities');
    Route::get('/pathao/zones', 'Front\CheckoutController@getPathaoZones')->name('front.pathao.zones');
    Route::get('/pathao/areas', 'Front\CheckoutController@getPathaoAreas')->name('front.pathao.areas');
    Route::get('/pathao/price', 'Front\CheckoutController@getPathaoPrice')->name('front.pathao.price');
    
    // AJAX endpoint to compute authoritative totals for current selection (multi-variant support)
    Route::post('/checkout/calculate-totals', 'Front\CheckoutController@calculateTotalsAjax')->name('front.checkout.calculate.totals');
        ...
        Route::get('/menualcheckout/{id}/{slug}', 'Front\CheckoutController@menualcheckout')->name('front.menualcheckout');
        ...
        // Temporary debug route: visit /_debug-checkout-submit to confirm HTTP routing is working.
        // Remove this after debugging.
        Route::get('/_debug-checkout-submit', function () {
